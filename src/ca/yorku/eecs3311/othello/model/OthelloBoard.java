 ...
 * 
 * Design Patterns: MVC Model holding board state; supports Visitor via accept().
 * 
 * @author student
 *
 */
public class OthelloBoard {
	...
	/**
	 * Construct a board from existing grid values.
	 */
	public OthelloBoard(char[][] boardData) {
		this.dim = boardData.length;
		this.board = new char[this.dim][this.dim];
		for (int r = 0; r < this.dim; r++) {
			for (int c = 0; c < this.dim; c++) {
				this.board[r][c] = boardData[r][c];
			}
		}
	}
	...
	public int getCount(char player) {
		CountVisitor visitor = new CountVisitor(player);
		this.accept(visitor);
	...
	*/
	public String toString() {
		// Use Visitor to render the board string
		RenderVisitor renderer = new RenderVisitor(this.dim);
	...
	/**
	 * Accept a visitor to traverse all cells (Visitor pattern).
	 */
	public void accept(BoardVisitor visitor) {
		for (int row = 0; row < this.dim; row++) {
			for (int col = 0; col < this.dim; col++) {
				visitor.visit(row, col, this.board[row][col]);
			}
		}
	}
}
